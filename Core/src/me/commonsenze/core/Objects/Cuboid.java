...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
	...
	public Cuboid(World world, Vector l1, Vector l2) {
	...
	/**
	 * Gets the ground location with reference to the location provided
	 * 
	 * @param loc - The reference 
	 * @return The location given from x and z with the y set to the ground
	 */
	public Location getGround(Location loc) {
		if (loc.getBlock().getType() == Material.AIR) {
			for (int y = loc.getBlockY(); y >= 60; y--) {
				loc.setY(y);
				if (loc.getBlock().getType() != Material.AIR&&loc.add(0,1,0).getBlock().getType() != Material.AIR)break;
			}
			loc.setY(loc.getY()+1.5);
		} else {
			for (int y = (loc.getBlockY() < 59 ? 60 : loc.getBlockY()); y< 256; y++) {
				loc.setY(y);
				if (loc.getBlock().getType() == Material.AIR&&loc.add(0,1,0).getBlock().getType() == Material.AIR)break;
			}
	 ...
	 * 
	 * @param loc - The reference 
	 * @return The random location from the lowest part of the Cuboid.
	 */
	public Location getRandomLocation() {
		Random random = new Random();
		int x = random.nextInt(getUpperX()-getLowerX())+getLowerX();
		int z = random.nextInt(getUpperZ()-getLowerZ())+getLowerZ();
		return new Location(getWorld(), x+0.5, getLowerY(), z+0.5);
	}
